## [2026-05-21 19:53] 🎯 AI 스캐너 탭(음식 인식 / OCR / 바코드)에 따른 가짜(Mock) 페이지 연동 완료
### 💬 진행 및 결정 사항 (Discussion)
- 사용자의 지시("사진에서 ocr 이랑 바코드가 있는데 이걸 관련된 가짜 페이지를 만들어야지")에 따라 스캐너 페이지 내 탭 동작 분기 처리 구현.
- **이미지 생성**: AI 이미지 생성 도구를 활용하여 '영양성분표(OCR용)' 사진과 '과자 뒷면 바코드(바코드용)' 사진을 `assets` 폴더에 생성 및 추가.
- **동적 UI 변경 구현 (`script.js`)**:
  - 스캐너 탭 클릭 시 뷰파인더(`vf-bg`)의 배경 사진과 안내 문구가 탭에 맞게 동적으로 변경됨.
  - 셔터를 눌렀을 때 실행되는 `triggerScan` 함수가 현재 탭 모드(`food`, `ocr`, `barcode`)를 인식하여 로딩 중 나타나는 스텝(step) 텍스트를 모드에 맞게 분기 처리함 (예: "이미지 내 텍스트 영역 감지 완료" 등).
  - 결과 페이지(`page-result`) 진입 시 모드별로 가짜 데이터(음식명, 칼로리, 이미지, 코멘트)를 삽입하도록 `updateResultMockData` 헬퍼 함수를 구축함.

### 🛠️ 코드 수정 내역 (Code Changes)
- **Added**: `assets/ocr_sample.png`, `assets/barcode_sample.png` (가짜 인식을 위한 샘플 이미지 리소스 추가)
- **Changed**: `index.html` — `scanner-mode-bar` 내 각 `span.smode` 요소에 `data-mode` 속성 추가
- **Changed**: `script.js` — 탭 클릭 시 뷰파인더 업데이트 로직 추가, `triggerScan()` 함수 내 로딩 텍스트 및 결과 데이터 주입 분기 처리 (`updateResultMockData` 구현)
